...
class DrawPad extends JComponent {
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < s.size(); i++) {
			if ((s.get(i).getClass().toString().equals("class Geometry.Line") || s
					.get(i).getClass().toString().equals("class Geometry.Triangle"))
					&& s.get(i).isSelected()) {
	...
	//            		Class classs;
				...
				if ((shape==6 && LoaderFileChooser.filen!="cancel") || shape!=6){
					CS.pressing(last);
					s.add(CS);
					current = s.size() - 1;
				}
			...
			for (int i = 0; i < s.size(); i++) {
				...
				if (s.get(i).isSelected() && i != current) {
					...
					if (lastPoint.get(i) == null) {//first time to add point
						...
						if(s.get(i).getClass().toString().equals("class Geometry.Triangle") || s.get(i).getClass().toString().equals("class Geometry.Line")){
							if(s.get(i).getClass().toString().equals("class Geometry.Triangle") && !s.get(current).getClass().toString().equals("class Geometry.Triangle")){		
							lastPoint
									.set(i,
											new Point(
													(int) (s.get(i).points[0]
															.getX()),
													(int) (s.get(i).points[0]
															.getY())));
								
							}else if(s.get(i).getClass().toString().equals("class Geometry.Triangle") && s.get(current).getClass().toString().equals("class Geometry.Triangle")) {
								lastPoint
								.set(i,
										new Point(
						...
						if(s.get(i).getClass().toString().equals("class Geometry.Line")){
							Line l = (Line)s.get(i);
							lastPoint
							.set(i,
									new Point(
						...
						if( !s.get(i).getClass().toString().equals("class Geometry.Triangle") && !s.get(i).getClass().toString().equals("class Geometry.Line")){
							...
							if(s.get(current).getClass().toString().equals("class Geometry.Triangle") &&( (point==1||point==2))){
								if(point!=1 && point!=2){
								n.y-=2*dy;
								}else{
									if(point==2){
							...
							dragging, point);
					s.set(i, sh);
						}else if(s.get(i).getClass().toString().equals("class Geometry.Triangle")){
							if(s.get(current).getClass().toString().equals("class Geometry.Triangle")){
								n.x+=dx;
								n.y+=dy;
								sh.drag(lastPoint.get(i), lastPoint.get(i),
										dragging, point);
							...
							lastPoint.set(i, n);
								...
								if(isMaximizing(dx,dy)){
									sh.drag(new Point((int)s.get(i).points[0].getX(),(int)s.get(i).points[0].getY()-1), lastPoint.get(i),
											dragging, 0);
								}else{
									sh.drag(new Point((int)s.get(i).points[0].getX(),(int)s.get(i).points[0].getY()+8), lastPoint.get(i),
											dragging, 0);
								}
							...
							Line l = (Line) s.get(i);
							double lengthBefore = new Point(l.x1,l.y1).distance(new Point(l.x2,l.y2));
							if(l.x1==l.x2){//vertical
								...
								Line l2 = (Line)sh;
								if(isMaximizing(dx,dy)){
									if(new Point(l2.x1,l2.y1).distance(l2.x2,l2.y2)-(lengthBefore)<0.000000001){
										n.y-=2*dy;
										lastPoint.set(i, n);
										sh.drag(lastPoint.get(i), lastPoint.get(i),
												dragging, 0);
		...
		private boolean isMaximizing(int dx,int dy) {
			if((point==0 && (dx<0|| dy<0))||(point==1 && dy<0)||(point==2 &&((dx>0 && dy==0)||(dx>0&&dy<0)||(dy<0&&dx==0)))||(point==3 && dx>=0)||(point==4 &&(dy>=0 || dx>=0))||(point==5 && dy>=0)||(point==6 && ((dx<0 && dy==0)||(dx<0&&dy>0)||(dy>0&&dx==0)))||(point==7&& dx<0) ){
				return true;
			}
			return false;
		}
